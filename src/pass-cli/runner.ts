 ...
 * - Preferred workflow: run `npm run pass:dev:output-policy:update` to regenerate
 *   this block from live `pass-cli --help` output, then verify with
 *   `npm run pass:dev:output-policy:check`.
 * - When adding/changing wrapped commands, update the command inventory in
 *   `scripts/sync-pass-cli-output-policy.mjs`.
